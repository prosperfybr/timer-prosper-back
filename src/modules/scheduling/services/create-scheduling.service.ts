...
export class CreateSchedulingService {
	...
	public async execute(payload: CreateSchedulingDTO): Promise<any> {
		...
		const { establishmentId, collaboratorId, serviceId, clientId, date, startTime, notes } = payload;
		...
		if (!establishment || !collaborator || !service || !client) {
			...
			throw new BadRequestException("Informações do agendamento incorretas para marcar um novo horário. Por favor revise e tente novamente");
		}
		...
		if (collaborator.establishmentId !== establishment.id) {
			...
			throw new BadRequestException("O colaborador informado não trabalha no estabelecimento informado");
		}

		/* 2. Verifica se o estabelecimento atende ao serviço informado */
		const establishmentMakeService: ServicesEntity[] = establishment.services.filter((service) => service.id === service.id);
		...
		const timeBlocks: AbsenceBlockEntity[] = await AbsenceBlockRepository.findAllByCollaboratorIdAndDate(
			collaborator.id,
		...
		const availablilitySlots: SchedulingResponse.SLOT[] = await this.findSchedulingService.findAvailableSlots(
			establishment.id,
			date,
			service.id,
			collaborator.id,
		...
		for (const slot of availablilitySlots) {
			...
			const slotStart = selectedDateTime.clone().startOf("day").add(moment.duration(slot.time));
			const slotEnd = selectedDateTime
				.clone()
				.startOf("day")
				.add(moment.duration(slot.time + service.duration));
		...
		const existingAppointments: AppointmentEntity[] = await AppointmentRepository.findAllByCollaboratorIdAndDate(
			collaborator.id,
			selectedDateTime.toDate(),
			endTime.toDate(),
		);
		for (const appointment of existingAppointments) {
			const appointmentRange = momentRange.range(moment(appointment.startTime), moment(appointment.endTime));

			if (proposedRange.overlaps(appointmentRange)) {
				log.error(`Already exist other appointement in this chose hour`);
				throw new BadRequestException("Conflito de horário com outro agendamento. Horário indisponível");
			}
		}
