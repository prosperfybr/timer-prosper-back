...
export class EstablishmentHourController {
	...
	public async findAllEstablishments(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const establishmentHours: EstablishmentHourResponseDTO = await this.findEstablishmentHourService.execute(establishmentId);
	...
	public async delete(req: Request, res: Response, next: NextFunction) {
		try {
			...
			return res.status(HttpStatusCode.Ok).json({ message: "Horário de funcionamento do estabelecimento deletado com sucesso." });
		} catch (error) {
