...
export const ClientEstablishmentRepository = AppDataSource.getRepository(ClientEstablishmentEntity).extend({
	...
	async findInviteByClientAndEstablishment(clientIdentifier: string, establishmentIdentifier: string):
	Promise<{
		client: UserEntity;
		owner: UserEntity;
		...
		FROM establishments as establishment
				...
				LEFT JOIN users client ON ${
					validateUUID(clientIdentifier) ?
					"client.id = '" + clientIdentifier + "'" :
					"client.email = '" + clientIdentifier + "'"
				}
