	...
	public async execute(createHour: CreateEstablishmentHourDTO): Promise<void> {
		...
		if (!hours || hours.length === 0) {
			...
			throw new InvalidArgumentException(
				'Os horários de abertura e fechamento do estabelecimento são obrigatórios',
			);
		}
		...
		hours.forEach(
			({
				...
				if (
					this.validatorUtils.validateHours(openingTime) &&
					this.validatorUtils.validateHours(closingTime)
				) {
